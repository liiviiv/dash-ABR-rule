module.exports = function (grunt) {
    ...
    //grunt的初始化配置工作：
    grunt.initConfig({
        pkg: grunt.file.readJSON('package.json'),//从package.json读取项目配置信息，并存入pkg属性
        //githash:get the current git hash, git branch and git tag
        githash: {
            options: {
                fail: false
            },
        ...
        uglify: {
            options: {
                banner: '/*! v<%= pkg.version %>-<%= githash.dist.short %>, <%= grunt.template.today("isoUtcDateTime") %> */',//压缩时添加的描述语句
                sourceMap: true,
                sourceMapIncludeSources: true,
                sourceMapRoot: './src/',//需压缩的文件所在路径
            ...
            build_core: {
                ...
                files: {
                    'build/temp/dash.mediaplayer.min.js': 'build/temp/dash.mediaplayer.debug.js'//压缩后的文件存储路径
                }
            },
            build_protection: {
                options: {
        ...
        //browserify:浏览器端代码模块化工具，一种预编译模块方案
        browserify: {
            //模块化，生成mediaplayer对象，可在其他地方引用
            mediaplayer: {
                files: {
            ...
            //watch:当检测到指定文件发现变化时，会按照制定的顺序执行制定的任务：
            watch: {
                files: {
                    'build/temp/dash.all.debug.js': ['index.js'],
                    'build/temp/dash.mss.debug.js': ['src/mss/index.js']
        ...
        jscs: {
            src: ['./src/**/*.js', 'test/unit/mocks/*.js', 'test/unit/*.js', 'Gruntfile.js'],//与jshint检查的相同文件
            options: {
                config: '.jscsrc'
            }
        },
    ...
    grunt.registerTask('lint', ['jshint', 'jscs']);//lint命令用于检查代码是否存在错误
